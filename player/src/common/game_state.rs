...
fn get_card_appearance(variety: u8, (x, y): (u8, u8)) -> CardAppearance {
    let variety = variety as u32;
...
fn card_entity_base_pos_to_grid_pos((x, y): (u8, u8)) -> (u8, u8) {
    ...
    let mut grid_y = if grid_x > END_OF_FOUNDATIONS {
        y - SECOND_ROW_ENTITY_Y
    } else {
        y
    };
            ...
            if input.pressed_this_frame(Button::Left) {
                ...
                if state.selectpos == BUTTON_COLUMN {
                    ...
                    if state.selectdrop {
                        ...
                        if candrop(&state.cells, grabpos, grabdepth, selectpos) {
    ...
    pub fn set_custom_state(&mut self, custom_state: CustomState) {
        ...
        for x in 0..CELLS_MAX_INDEX as usize + 1 {
            ...
            for y in 0..column.len() {
                ...
                let full_entity = get_card_full_entity(variety, get_card_pos((x as u8, y as u8)));
...
fn get_card_full_entity(variety: Variety, pos: (u8, u8)) -> FullEntity {
...
fn card_entity_pos_to_grid_pos(pos: [Position; GameState::ENTITY_PIECE_COUNT]) -> (u8, u8) {
